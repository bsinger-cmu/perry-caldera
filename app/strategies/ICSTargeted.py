...
from plugins.deception.app.models.network import Network, Subnet, AttackPath
...
class LogicalPlanner(PerryStrategy):
    def __init__(
        ...
        self.environment_state_service = EnvironmentStateService(
            self.knowledge_svc_handle, operation
        )
    ...
    def choose_attack_path(self):
        ...
        for attack_path in self.attack_path_queue:
            if attack_path.target_host.hostname in self.criticalHostNames:
                # fmt: off
            ...
            if attack_path.attack_technique.CredentialToUse is not None:
                ...
                if (
                    path_hostname in self.criticalHostNames
                    or "controlhost" in path_hostname.lower()
                ):
                    # fmt: off
